...
/**
 * A Preact component to handle user authentication (Login, Logout, Status).
 * Renders a login modal when "Login" is clicked.
 */
export default function Auth() {
  ...
  // ... (useEffect block remains unchanged) ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (_event, session) => {
        setSession(session);
        setShowLogin(false); // Close modal on login/logout
        setLoading(false);
        setMessage('');
        setEmail('');
        setPassword('');
  ...
  // ... (handleLogin and handleLogout remain unchanged) ...
  const handleLogin = async (e: h.JSX.TargetedEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    setMessage('');
