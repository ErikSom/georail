var PathFinder = pc.createScript('pathFinder');

PathFinder.attributes.add('railData', {type: 'asset', assetType:'json'});



PathFinder.prototype.createCoordinateToNodeDataMap = function(originalFeatures) {
    const coordinateMap = {};
...
PathFinder.prototype.initialize = function() {
    ...
    this.pathFinder = new geojsonPathFinder(this.railData.resource);
    this.coordinateToNodeDataMap = this.createCoordinateToNodeDataMap(this.railData.resource.features);

    this.nodes = this.railData.resource.features.filter(feat => feat.properties['@id'].startsWith('node'));
    this.lines = this.railData.resource.features.filter(feat => feat.geometry?.type === "LineString");
    this.nameStopLookup = {};
    ...
    window.pathFinder = this;
};

PathFinder.prototype.setupQuadTree = function(){
    const xGet = d => d.x;
    const yGet = d => d.y;
    const zGet = d => d.z;
...
PathFinder.prototype.queryQuadTree = function(position, radius){
...
PathFinder.prototype.queryNearbyLineCoords = function(refNode, distance){
...
PathFinder.prototype.coordKey = function(coord, tolerance = 1e-5){
       return `${Math.round(coord[0] / tolerance) * tolerance},${Math.round(coord[1] / tolerance) * tolerance}`;
}

PathFinder.prototype.getStationNode = function(name, track){
    if(Array.isArray(this.nameStopLookup[name])){
...
PathFinder.prototype.findPath = function(a, b){
    return this.pathFinder.findPath(a, b);
}

PathFinder.prototype.getCoordFromNode = function(node){
    if(node._pcCoord){
...
PathFinder.prototype.getPositionFromCoordinate = function(lon, lat, alt = 300){
    const [x, y, z] = earthatile.geodeticToCartesian(lon, lat, alt);
    return this.v1.set(x, z, -y);
}

PathFinder.prototype.getDataForCoord = function(coord){
    const key = this.coordKey(coord)
    return this.coordinateToNodeDataMap[key];
}

PathFinder.prototype.getFloorPositionFromNode = function(node){
